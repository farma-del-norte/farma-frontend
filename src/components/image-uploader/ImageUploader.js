...
const ImageUploader = ({base64Images = [], handleImages, multiple = true}) => {
  ...
  const handleDrop = e => {
    e.preventDefault()
    if (!multiple) return setImages([...e.dataTransfer.files])
  ...
  const handleFileInputChange = e => {
    e.preventDefault()
    if (!multiple) return setImages([...e.target.files])
    const newImages = [...images]
  ...
  return (
    ...
    >
      <Typography variant='body'>Para subir imágenes arrastralas aquí o agregalas a través del botón</Typography>
      <input
        type='file'
        accept='.jpg,.png,.webp'
