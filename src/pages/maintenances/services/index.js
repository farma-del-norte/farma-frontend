...
const Services = () => {
  ...
  return (
    <Fragment>
                    ...
                    <>
                      ...
                      >
                        {serviceCat?.map((servCat, id) => (
                          <MenuItem key={id} value={servCat.id}>
                            {servCat.name}
                          </MenuItem>
                        ))}
                      </Select>
                      ...
                      >
                        {suppliers?.map((supplier, id) => (
                          <MenuItem
                            key={id}
                            value={supplier.id}
                          >{`${supplier.firstname} ${supplier.lastname}`}</MenuItem>
                        ))}
                      </Select>
                    </>
                  )}
                ...
                <Controller
                  ...
                  render={({field: {value, onChange}}) => (
                    <>
                      ...
                      >
                        {status?.map((statu, id) => (
                          <MenuItem key={id} value={statu.name}>
                            {statu.name}
                          </MenuItem>
                        ))}
                      </Select>
            ...
            <Grid item xs={12} md={6} sx={{marginTop: '6px'}}>
              <FormControl fullWidth>
                <Controller
                  ...
                  render={({field: {value, onChange}}) => (
                    <TextField
                      label={t('services.columns.cost', {ns: 'maintenances'})}
                      value={value}
                      onChange={onChange}
                      InputProps={{
                        startAdornment: (
      ...
      <MaterialsModal isOpen={isModalOpen} control={control} handleSubmit={handleSubmit}>
        <ExpandedContent label={t('services.expanded_title', {ns: 'maintenances'})}>
          {isModalOpen && (
            <Grid container>
              <DetailTextFieldForm
                labelText={t('services.columns.serviceCat', {ns: 'maintenances'})}
                value={serviceRow.service.serviceCatName}
              />
              <DetailTextFieldForm
                labelText={t('services.columns.area_type', {ns: 'maintenances'})}
                value={serviceRow.service.area}
              />
              {areaContent.length > 0 ? (
                <DetailTextFieldForm
                  md={6}
                  labelText={t('services.columns.area', {ns: 'maintenances'})}
                  value={getValueFromID(areaContent, serviceRow.service.areaID, 'name')}
                />
              ) : (
                <ClipLoader />
              )}
              <DetailTextFieldForm
                labelText={t('services.columns.cost', {ns: 'maintenances'})}
                value={serviceRow.service.cost}
              />
              <DetailTextFieldForm
                labelText={t('services.columns.status', {ns: 'maintenances'})}
                value={serviceRow.service.status}
              />
              {suppliers.length > 0 ? (
                <DetailTextFieldForm
                  md={6}
              ...
              <DetailTextFieldForm
                md={6}
                multiline={true}
                labelText={t('services.columns.notes', {ns: 'maintenances'})}
                value={serviceRow.service.notes}
              />
            </Grid>
          )}
        </ExpandedContent>
